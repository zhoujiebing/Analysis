...
def send_add_order_and_support(article_code_list, date):
    """发送每日新增订单与服务支持"""
    
    merge_order = []
    merge_support = []
    order_head = CURRENT_DIR+'data/order/order_head.csv'
    ...
    for article_code in article_code_list:
        merge_order.append(CURRENT_DIR+'data/order/order_'+article_code+str(date)+'.csv')
        merge_support.append(CURRENT_DIR+'data/support_'+article_code+'.csv')
    ...
    subject = '昨日新增订单'
    content = '注意，订单抓取现在使用新脚本且近期订单API抽风的可能性比较大，请事先认真核对下再导入到百会CRM。\n有问题和不明确的请及时反馈'
...
if __name__ == '__main__':
    try: 
        ...
        send_add_order_and_support(['ts-1796606', 'ts-1797607'], date)
    except Exception, e:
